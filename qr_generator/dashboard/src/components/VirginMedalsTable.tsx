...
const VirginMedalsTable: React.FC<VirginMedalsTableProps> = ({
  ...
  const [sortField, setSortField] = useState<keyof VirginMedal>('created_at');
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th 
                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                onClick={() => handleSort('medal_string')}
              >
                <div className="flex items-center space-x-1">
                  <span>Medal String</span>
                  <SortIcon field="medal_string" />
                </div>
              </th>
              <th 
                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                onClick={() => handleSort('register_hash')}
              >
                <div className="flex items-center space-x-1">
                  <span>Register Hash</span>
                  <SortIcon field="register_hash" />
                </div>
              </th>
              <th 
                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
            ...
            {currentPageMedals.map((medal) => (
              <tr key={medal.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {medal.created_at ? new Date(medal.created_at).toLocaleString('es-ES') : 'N/A'}
                </td>
